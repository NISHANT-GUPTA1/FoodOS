      ...
      <ul className="flex flex-wrap justify-center gap-6 p-4">
        {menuItems.map(({ title, icon, gradientFrom, gradientTo }, idx) => (
          ...
          >
            ...
            {/* Title */}
            <span className="absolute z-10 text-white font-bold uppercase tracking-wide text-sm transition-all duration-500 scale-0 group-hover:scale-100 delay-150 whitespace-nowrap">
              {title}
            </span>
          </li>
        ))}
